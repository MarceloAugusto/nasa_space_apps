...
@Component({
  selector: 'app-batalha-poluicao',
  standalone: true,
  imports: [CommonModule, IonContent, IonButton, IonHeader, IonToolbar, IonTitle],
  templateUrl: './batalha-poluicao.page.html',
  styleUrls: ['./batalha-poluicao.page.scss'],
})
export class BatalhaPoluicaoPage {
  // === Sprites (coloque as imagens conforme seus arquivos)
  heroSrc    = 'assets/heroi-fazendeiro.png';
  villainSrc = 'assets/vilao-poluicao.png';

  private router = inject(Router);

  // === Perguntas (dados da NASA sobre poluição e seus efeitos)
  questions: Question[] = [
   { q: 'Which NASA satellite helps monitor pollution hotspots in rural areas?',
    a: ['FIRMS', 'SMAP', 'GPM', 'MODIS'], correct: 0 },
  { q: 'What does FIRMS (Fire Information for Resource Management) provide?',
    a: ['Real-time data on fires', 'Soil moisture data', 'Weather forecasts', 'Solar radiation measurements'], correct: 0 },
  { q: 'Which data are used to monitor the extent of fires during burn events?',
    a: ['MODIS', 'NDVI', 'LST', 'GPM'], correct: 0 },
  { q: 'What is the impact of fires and pollution on agriculture?',
    a: ['Destruction of crops and soil', 'Increase in humidity', 'Improvement in soil quality', 'Increase in food production'], correct: 0 },
  { q: 'How does GPM (Global Precipitation Measurement) help combat pollution and fires?',
    a: ['By providing data on precipitation and humidity', 'By measuring soil temperature', 'By monitoring air pollutants', 'By calculating wind speed'], correct: 0 },
  ];
  ...
  damaFogo(){
    this.router.navigateByUrl('/batalha-fogo');
  }
}
